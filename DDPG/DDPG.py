...
class DDPG:
    ...
    def learn(self):
        ...
        rewards_tensor = torch.tensor(reward, dtype=torch.float)
        next_states_tensor = torch.tensor(states_, dtype=torch.float)
        ...
        new_action_tensor = self.actor.forward(states_tensor)
        actor_loss = -torch.mean(self.critic(states_tensor, new_action_tensor))
