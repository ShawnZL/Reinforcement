...
class COMA:
    def __init__(self, args):
        ...
        self.target_critic1 = ComaCritic(critic_input_shape, self.args)
        ...
        self.target_critic1.load_state_dict(self.eval_critic.state_dict())
    ...
    def _get_q_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            q_target1 = self.target_critic1(inputs_next)

            # 把q值的维度重新变回(episode_num, n_agents, n_actions)
            q_eval = q_eval.view(episode_num, self.n_agents, -1)
            q_target = q_target.view(episode_num, self.n_agents, -1)
            q_target1 = q_target1.view(episode_num, self.n_agents, -1)
            critic_q = torch.min(q_target, q_target1)
            q_evals.append(q_eval)
            q_targets.append(critic_q)
